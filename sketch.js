...
let graphics;  // Declare graphics object
...
function setup() {
    ...
    // Create an off-screen graphics object with the same size as the canvas
...
function draw() {
    ...
    if (brushType === "point") {
        ...
        let numRays = 12; // Number of lines in the firework burst
...
function togglePopup() {
    ...
    if (popupVisible) {
        // Animate to top-right corner and scale down
        popup.style.transform = 'translate(-50%, -50%) scale(0)';
        popup.style.top = '7.8%'; // Set top position to original top-right
        popup.style.left = '96%'; // Set left position to original top-right
        setTimeout(() => {
            popup.style.display = 'none'; // Hide the popup after animation
        }, 500); // Match the duration of the transition
    } else {
        // Show popup, animate to center, and scale up
        popup.style.display = 'block';
        setTimeout(() => {
            popup.style.transform = 'translate(-50%, -50%) scale(1)'; // Scale up
            popup.style.top = '50%'; // Center vertically
            popup.style.left = '50%'; // Center horizontally
        }, 10); // Slight delay to ensure transition applies
    }

    popupVisible = !popupVisible; // Toggle visibility state
}
...
function keyPressed() {
    ...
    // Save button with command S
      ...
      // Draw the white background to the graphics buffer
      graphics.background(255);

      // Copy the current content from the main canvas to the graphics buffer
      graphics.image(get(), 0, 0);  // Copy the entire canvas into the buffer
      
      // Now save the image from the buffer
      save(graphics, 'MyDrawing.png');
      }
}
